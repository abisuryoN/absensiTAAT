<x-app-layout>
    ...
    <div class="row mb-4">
        <div class="col">
            ...
            <h3 class="fw-bold tracking-tight text-dark mb-1">Edit Semester</h3>
            <p class="text-muted mb-0">Ubah informasi semester sekolah.</p>
        </div>
    </div>
                ...
                <div class="card-body p-4">
                    <form method="POST" action="{{ route('admin.semesters.update', $semester) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="Contoh: Ganjil atau Genap" value="{{ old('name', $semester->name) }}" required>
                            @error('name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="semester_number" id="semester_number" class="form-select @error('semester_number') is-invalid @enderror" required>
                                <option value="1" {{ old('semester_number', $semester->semester_number) == 1 ? 'selected' : '' }}>Semester 1 (Ganjil)</option>
                                <option value="2" {{ old('semester_number', $semester->semester_number) == 2 ? 'selected' : '' }}>Semester 2 (Genap)</option>
                            </select>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="date" name="start_date" id="start_date" class="form-control @error('start_date') is-invalid @enderror" value="{{ old('start_date', $semester->start_date->format('Y-m-d')) }}" required>
                                @error('start_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="date" name="end_date" id="end_date" class="form-control @error('end_date') is-invalid @enderror" value="{{ old('end_date', $semester->end_date->format('Y-m-d')) }}" required>
                                @error('end_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="mb-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $semester->is_active) ? 'checked' : '' }}>
