<x-app-layout>
    ...
    <div class="row mb-4">
        <div class="col">
            ...
            <h3 class="fw-bold tracking-tight text-dark mb-1">Edit Hari Libur</h3>
            <p class="text-muted mb-0">Ubah detail hari libur sekolah.</p>
        </div>
    </div>
                    ...
                    <form method="POST" action="{{ route('admin.holidays.update', $holiday) }}">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="date" name="date" id="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $holiday->date->format('Y-m-d')) }}" required>
                                @error('date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" rows="3">{{ old('description', $holiday->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
